package br.edu.ifba.aem.domain.utils.cuid;

import br.edu.ifba.aem.domain.utils.cuid.exception.CUIDGenerationException;
...
public final class CUID implements Serializable, Comparable<CUID> {
  ...
  public static CUID randomCUID2(final int length) {
    ...
    final char firstLetter = CUIDv2.ALPHABET_ARRAY[safeAbs(Common.nextIntValue())
        % CUIDv2.ALPHABET_ARRAY.length];
    final String hash = CUIDv2.computeHash(
        time + CUIDv2.createEntropy(length) + CUIDv2.nextCounterValue()
            + Common.MACHINE_FINGERPRINT, length);
  ...
  public static CUID randomCUID1() {
    ...
    final String counter = Common.padWithZero(
        Integer.toString(CUIDv1.nextCounterValue(), NUMBER_BASE), CUIDv1.BLOCK_SIZE);
    final String random = CUIDv1.getRandomBlock() + CUIDv1.getRandomBlock();

    return new CUID(
        CUIDv1.START_CHARACTER + timestamp + counter + Common.MACHINE_FINGERPRINT + random);
  }
  ...
  public static boolean isValid(final String cuidAsString) {
    ...
    return cuidAsString != null && (
        cuidAsString.length() == CUIDv1.LENGTH_STANDARD && cuidAsString.startsWith(
            CUIDv1.START_CHARACTER) // Version 1
  ...
  private static final class CUIDv1 {
    ...
    private static String getRandomBlock() {
      ...
      return Common.padWithZero(
          Integer.toString(Common.nextIntValue() * DISCRETE_VALUE, NUMBER_BASE), BLOCK_SIZE);
    }
  }
  ...
  private static final class CUIDv2 {
    ...
    private static final char[] ALPHABET_ARRAY = new char[]{'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h',
        'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'};
    private static final int[] PRIME_NUMBER_ARRAY = new int[]{109717, 109721, 109741, 109751,
        109789, 109793, 109807, 109819, 109829, 109831};
    ...
    private static String createEntropy(final int length) {
      ...
      while (stringBuilder.length() < length) {
        primeNumber = PRIME_NUMBER_ARRAY[safeAbs(Common.nextIntValue())
            % PRIME_NUMBER_ARRAY.length];
    ...
    private static String computeHash(final String content, final int saltLength) {
      ...
      try {
        return new BigInteger(MessageDigest.getInstance("SHA3-256")
            .digest((content + salt).getBytes(StandardCharsets.UTF_8))).toString(NUMBER_BASE);
